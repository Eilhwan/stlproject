			...
			<tfoot>
				...
				<tr>
					<td colspan="4" class="text-center"><c:if
							test="${startPage > 10 }">
							<a href="${conPath }/eventBoardView.do?pageNum=${startPage - 1 }">[이전]</a>
						</c:if> <c:forEach var="i" begin="${startPage }" end="${endPage }">
							<c:if test="${pageNum eq i }">
								<b>${i }</b>
							</c:if>
							<c:if test="${pageNum != i }">
								<a href="${conPath }/eventBoardView.do?pageNum=${i }">[${i }]</a>
							</c:if>

						</c:forEach> <c:if test="${endPage < pageCnt}">
							<a href="${conPath }/eventBoardView.do?pageNum=${endPage + 1 }">[다음]</a>
						</c:if></td>
				</tr>
			</tfoot>
		</table>
